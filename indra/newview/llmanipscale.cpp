...
void LLManipScale::dragCorner( S32 x, S32 y )
{
	...
	F32 prim_scale;
	if (gSavedSettings.getBOOL("VHMegaprims"))
	{
...
void LLManipScale::stretchFace( const LLVector3& drag_start_agent, const LLVector3& drag_delta_agent )
{
		 ...
		 iter != mObjectSelection->end(); iter++)
	{
		...
		if( cur->permModify() && cur->permMove() && !cur->isAvatar() )
		{
			...
			F32 desired_scale		= llclamp(selectNode->mSavedScale.mV[axis_index] + desired_delta_size, MIN_PRIM_SCALE, (gSavedSettings.getBOOL("VHMegaprims") ? DEFAULT_MAX_MEGAPRIM_SCALE : DEFAULT_MAX_PRIM_SCALE));
...
F32		LLManipScale::partToMaxScale( S32 part, const LLBBox &bbox ) const
{
	...
	max_scale_factor = bbox_extents.magVec() * (gSavedSettings.getBOOL("VHMegaprims") ? DEFAULT_MAX_MEGAPRIM_SCALE : DEFAULT_MAX_PRIM_SCALE) / max_extent;
...
F32		LLManipScale::partToMinScale( S32 part, const LLBBox &bbox ) const
{
	LLVector3 bbox_extents = unitVectorToLocalBBoxExtent( partToUnitVector( part ), bbox );
	bbox_extents.abs();
	F32 min_extent = (gSavedSettings.getBOOL("VHMegaprims") ? DEFAULT_MAX_MEGAPRIM_SCALE : DEFAULT_MAX_PRIM_SCALE);
	for (U32 i = VX; i <= VZ; i++)
	{
