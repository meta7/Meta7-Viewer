...
void LLToolPie::leftMouseCallback(const LLPickInfo& pick_info)
{
	...
	if(gSavedSettings.getBOOL("VHClickSendTouchPos"))
	{
...
// [/RLVa:KB]
		...
		case CLICK_ACTION_SIT:
			if ((gAgent.getAvatarObject() != NULL) && (!gAgent.getAvatarObject()->mIsSitting) && !gSavedSettings.getBOOL("VHBlockClickSit")) // agent not already sitting
			{
				handle_sit_or_stand();
				return TRUE;
...
BOOL LLToolPie::handleDoubleClick(S32 x, S32 y, MASK mask)
{
	...
	if (gSavedSettings.getBOOL("DoubleClickAutoPilot") || gSavedSettings.getBOOL("VHDoubleClickTeleport"))
	{
		if (mPick.mPickType == LLPickInfo::PICK_LAND
